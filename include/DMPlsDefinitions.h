...
std::vector <std::vector<std::vector<double>>>npk_x;
std::vector <std::vector<std::vector<double>>>npk_y;
...
void first_initializer(int dim, int npol, int *gr_imt)
{
  ...
  for(int in=0; in<dim;in++){
    ...
    npk_x[in].resize(npol);
    npk_y[in].resize(npol);
    npk_npx[in].resize(npol);
    npk_vol[in].resize(npol);
