...
void start_v2(){
  ...
  gSystem->Exec("cp $DMPLA/macros/myData_v6.C .");
  gSystem->Exec("cp $DMPLA/include/DMPlsDefinitions.h .");
  gSystem->Exec("cp $DMPLA/macros/settings.mac .");
  //gSystem->Exec("cp $DM_ROOT/src/macros/run_gif.C .");
  gSystem->Exec("cp $DMPLA/macros/node.sh .");
  
  TString dir = gSystem->UnixPathName(__FILE__);
  dir.ReplaceAll("start_v2.C","");
  dir.ReplaceAll("/./","/");
  ifstream in;
